...
client.on("messageCreate", async (message) => {
  ...
  if (message.channel.id === config.addBotChannel) {
    ...
    const isAskingHelp = (words) => {
      for (const word in words) {
        if (message.content.toLowerCase().includes(words[word].toLowerCase())) return true;
        else return false;
      }
    };
    const wrongFormat = (reason) => {
      message.delete();
      message.author.send(
        `Your request is not in the right format. Please retry. Reason: ${reason}`
      );
    };
    if (!clientIDLine || !sourceCodeLine)
      return wrongFormat("cannot parse message");
    const clientID = clientIDLine
      .slice("Client ID:".length, clientIDLine.length)
      .trim();
    if (!clientID || !/^([0-9]{1,32})$/.test(clientID))
      return wrongFormat("client ID not found");
    const sourceCode = sourceCodeLine
      .slice("Source code URL:".length, sourceCodeLine.length)
      .trim();
    ...
    /s4d|scratch/.test(message.content) &&
    message.channel.id !== config.scratchForDiscordChannelID
  ) {
    ...
    isAskingHelp(helpWords) &&
      config.chatChannelIDs.includes(message.channel.id))
  ) {
    message.channel.send({
...
client.on("guildMemberAdd", (member) => {
  if (member.user.bot) {
    member.roles.add(
      member.guild.roles.cache.find((role) => role.name === "Bots")
    );

    const addBotChannel = client.channels.cache.get(config.addBotChannel);
    addBotChannel.messages.fetch().then((messages) => {
      const addMessage = messages.some((m) =>
        m.embeds[0]?.description.startsWith(`**ID:** ${member.user.id}`)
      );
      if (addMessage) addMessage.delete();
    });
  } else {
    member.roles.add(
      member.guild.roles.cache.find((role) => role.name === "Members")
    );
  }
